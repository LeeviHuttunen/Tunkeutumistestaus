...
- Tässä kohtaa pidin parin tunnin tauon ja jatkoin sitten. En saanut enää yhteyttä jostain syystä samaan databaseen, joten jouduin poistamaan vanhan databasen ja tekemään uuden tilalle komennolla `sudo msfdb reinit`. Myöhemmin ymmärsin, että database pitää käynnistää uudelleen Kalissa komennolla `sudo systemctl start postgresql`
...
- Siellä näkyy kohta 0 ja 1. Muistaakseni viime oppitunnilla puhuttiin, että komento `use 0` toimii tässä tilanteessa. Laitoin `setg RHOSTS` ja vielä hyökättävän koneen ip:n perään Teron ohjeiden mukaan. Sen jälkeen kirjoitin exploit:
...
- Tämän jälkeen kokeilin vielä huumorilla noin viidennentoista kerran, että pääsenkö kirjautumaan sisään tehtävän g komennoilla. Ja nyt jostain tuntemattomasta syystä pääsin taas sisään:
